...
namespace CheckerApp.Application.Hardwares.Commands.UpdateHardware
{
    public class UpdateHardwareCommandHandler : IRequestHandler<UpdateHardwareCommand>
    {
        ...
        {
            var entity = await _context.Hardwares.FindAsync(new object[] { request.Hardware.Id }, cancellationToken);
            ...
            switch (hardware.HardwareType)
            {
                ...
                case HardwareType.Network:
                    ...
                    var networkDevices = ((NetworkHardwareDto)hardware).NetworkDevices.Select(dto =>
                    new NetworkDevice
                    {
                        Name = dto.Name,
                        IP = dto.IP,
                ...
                case HardwareType.ARM:
                    ...
                    var adapters = ((ArmDto)hardware).NetworkAdapters.Select(dto => new NetworkAdapter
                    {
                        IP = dto.IP,
                        MacAddress = dto.MacAddress
                    }).ToHashSet();
